...
- 每次處理需求前，先讀本檔。
- 每次讀取 `AGENTS.md` 時，同步載入分析快取 `cache_analysis/analysis-cache.md`。
...
- [`.agent_customs/rules/form/hahaha_form_test.md`](.agent_customs/rules/form/hahaha_form_test.md)（預留，僅目標頁面存在時讀）
- [`.agent_customs/rules/form/hahaha_form_setting.md`](.agent_customs/rules/form/hahaha_form_setting.md)（預留，僅目標頁面存在時讀）
...
3. `.agents/AGENTS.md`
4. 需求對應規則檔
5. `c_sharp_base_v2.sln`
6. 需求直接相關的 `.csproj`
7. `hahahalib` / `hahahaUIlib` 對應模組
8. 最後才打開 `c_sharp_base_v2` 與 `project/` 內直接相關的 form、box、flow、thread、流程檔
